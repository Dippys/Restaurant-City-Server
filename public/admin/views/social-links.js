...
export async function render(container) {
    ...
    container.append(h('div', { class: 'rc-panel-head' }, h('div', {}, h('h1', { class: 'rc-title' }, 'Social links'), h('p', { class: 'rc-sub' }, 'Create, preview, schedule, limit, pause, revoke, duplicate and audit campaigns.')), create), table(['Campaign', 'Kind', 'Status', 'Claims', 'Schedule', 'Actions'], links.map((link) => [
...
function actionBar(link, container) {
    const bar = h('div', { class: 'rc-toolbar' });
    const add = (label, fn) => bar.append(also(h('button', { class: 'rc-btn small', type: 'button' }, label), (button) => button.addEventListener('click', fn)));
    ...
    if (link.status === 'DRAFT')
        add('Activate', () => void lifecycle(link.id, 'activate', container));
    if (link.status === 'ACTIVE') {
        add('Pause', () => void lifecycle(link.id, 'pause', container));
        add('Expire', () => void lifecycle(link.id, 'expire', container));
    }
    if (link.status === 'PAUSED')
        add('Resume', () => void lifecycle(link.id, 'resume', container));
    if (link.status !== 'REVOKED')
        add('Revoke', () => void lifecycle(link.id, 'revoke', container));
    add('Duplicate', () => void lifecycle(link.id, 'duplicate', container));
    return bar;
}
async function lifecycle(id, operation, container) {
    try {
        await api.socialLifecycle(id, operation);
        toast(`Campaign ${operation}d`);
