...
async function openMailComposer() {
    try {
        ...
        const recipientSelect = h('select', { class: 'rc-input rc-mail-recipients', multiple: true, size: 9 }, ...users.map((user) => h('option', { value: user.networkUid }, `${user.fullName || user.firstName || user.networkUid} (${user.networkUid})`)));
        const recipientLabel = h('label', {}, 'Specific people', recipientSelect, h('small', { class: 'rc-note' }, 'Use Ctrl/Cmd or Shift to select several people.'));
