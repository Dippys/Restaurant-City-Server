...
import { also, confirmDialog, fmt, h, toast } from '../ui.js';
const PAGE_SIZE = 50;
...
export async function render(container) {
    ...
    const toolbar = h('div', { class: 'rc-toolbar' }, filterInput, h('label', { class: 'rc-check' }, liveBox, ' live (stick to newest)'));
    const footer = h('div', { class: 'rc-traffic-foot' }, countLabel, pageLabel, prevBtn, nextBtn, newestBtn, also(h('button', { class: 'rc-btn small', type: 'button' }, 'Copy all'), (btn) => {
        ...
        btn.addEventListener('click', async () => {
            if (!(await confirmDialog('Clear the request buffer?', 'The live log will empty; new requests keep arriving.', false)))
                return;
    ...
    container.textContent = '';
    const view = h('div', { class: 'rc-view-fill' }, h('h1', { class: 'rc-title' }, 'Live traffic'), h('p', { class: 'rc-sub' }, 'Every request the game and players make, in real time — paginated, newest first.'), toolbar, list, footer);
    ...
    source.onmessage = (event) => {
        try {
            ...
            entries.push(entry);
            if (entries.length > MAX_BUFFER)
                entries = entries.slice(-MAX_BUFFER);
            if (live && page === 0) {
                renderList();
            }
            else {
...
function applyFilter(all) {
    if (!filterValue)
        return all;
    return all.filter((entry) => `${entry.method} ${entry.path} ${entry.rpc?.call ?? ''} ${entry.account?.username ?? ''}`.toLowerCase().includes(filterValue));
}
function totalPages() {
...
function renderList() {
    ...
    if (prev)
        prev.style.visibility = page < pages - 1 ? 'visible' : 'hidden';
    if (next)
        next.style.visibility = page > 0 ? 'visible' : 'hidden';
    if (newest)
        newest.style.visibility = page === 0 && live ? 'hidden' : 'visible';
}
function row(entry) {
    ...
    const head = h('div', { class: 'rc-request-head' }, h('span', { class: `rc-method ${statusClass}` }, method), h('span', { class: 'rc-status' }, String(entry.status)), entry.account ? h('span', { class: 'rc-user', title: `uid ${entry.account.networkUid}` }, entry.account.username) : null, h('span', { class: 'rc-summary' }, summary), h('span', { class: 'rc-time' }, time), entry.bodyLen ? h('span', { class: 'rc-size' }, `${fmt(entry.bodyLen)}B`) : null);
...
function detailText(entry) {
    ...
    if (entry.rpc?.subs?.length) {
        ...
        for (const sub of entry.rpc.subs)
            lines.push(`  ${sub.name}  ${sub.answered}`);
    }
    if (entry.bodyText)
        lines.push('body:', entry.bodyText.slice(0, 4000));
