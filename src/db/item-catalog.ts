...
export function isGiftableItemId(id: number): boolean {
  ...
  // Perk subtypes 602/603 are coin/popularity reward tokens, not transferable
  // inventory. Other fanPageFeed rows can be real items (including ingredients),
  // so the subtype check is intentionally narrow.
  if ([602, 603].includes(Math.floor(id / 10_000)) && attrs.fanPageFeed === 'true') return false;
