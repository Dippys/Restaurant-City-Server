...
async function renderQueue(container: HTMLElement): Promise<void> {
  ...
  const resolveButton = also(h('button', { class: 'rc-btn primary', type: 'button' }, 'Resolve all signals'), (button) => button.addEventListener('click', async () => {
    if (!(await confirmDialog('Resolve all signals?', 'For every profile with an open staff/menu/level signal: fire staff beyond the level cap, deselect dishes beyond the course cap, and catch the level up to its gourmet value. Each profile gets a recovery snapshot and an audit entry first.', true))) return;
    button.disabled = true;
    try {
  ...
  container.append(h('h1', { class: 'rc-title' }, 'Anomaly review'), h('p', { class: 'rc-sub' }, 'Explainable evidence only. No finding automatically bans, deletes, or rolls back a player.'),
    ...
    h('div', { class: 'rc-toolbar' }, search, severity, scanButton, resolveButton, resetButton), panel);
  redraw();
}
