...
function itemField(key: string, label: string, help = 'Type an item name, or pick from the suggestions.'): FieldSpec {
...
async function renderList(container: HTMLElement): Promise<void> {
  ...
  const redraw = () => {
    ...
    listBox.append(
      table(
        ['UID', 'Player', 'Restaurant', 'Lv', 'Gourmet', 'Coins', 'Cash', 'Updated', ''],
        visible.map((user) => [
          user.networkUid,
          ...
          h('span', { class: 'rc-row-actions' },
            also(h('button', { class: 'rc-btn small', type: 'button' }, 'Open'), (btn) => {
...
function openCreateUser(onDone: () => void): void {
  const form = renderForm(PROFILE_FIELDS, {}, async (values) => {
    try {
      await api.createUser(values as never);
...
// ---------- detail ----------
async function renderDetail(container: HTMLElement, networkUid: string): Promise<void> {
  container.textContent = 'Loading…';
  ...
  if (!user) {
    container.textContent = '';
    container.append(
      h('p', { class: 'rc-err' }, `Player ${esc(networkUid)} not found.`),
      also(h('button', { class: 'rc-btn', type: 'button' }, '← Back to players'), (btn) => btn.addEventListener('click', () => (window.location.hash = '#/users'))),
    );
    return;
  }
  ...
  const header = h('section', { class: 'rc-panel rc-profile' },
    h('div', { class: 'rc-profile-id' },
      ...
      h('p', { class: 'rc-sub' }, `UID ${user.networkUid} · playfish ${fmt(user.playfishUid)} · ${esc(user.restaurantName)}`),
      h('div', { class: 'rc-badges' },
        badge(`Lv ${user.userLevel}`, 'ok'),
        badge(user.isInStreet ? 'in street' : 'in restaurant', 'muted'),
        badge(user.activeFloorIndex > 0 ? `floor ${user.activeFloorIndex}` : 'ground floor', 'muted'),
        badge(`${fmt(user.credits)} coins`, 'ok'),
        badge(`${fmt(user.cashBalance)} cash`, 'warn'),
        badge(`${fmt(user.gourmetPoint)} gourmet`, 'muted'),
        badge(`play count ${fmt(user.playCount)}`, 'muted'),
        badge(`updated ${relTime(Math.floor(new Date(user.updatedAt).getTime() / 1000))}`, 'muted'),
      ),
    ),
    h('div', { class: 'rc-row-actions' },
      ...
      also(h('button', { class: 'rc-btn danger', type: 'button' }, 'Delete player'), (btn) => {
        btn.addEventListener('click', async () => {
          if (!(await confirmDialog('Delete player?', `${user!.networkUid} and all their data will be deleted permanently.`, true))) return;
          await api.deleteUser(user!.networkUid);
          toast('Player deleted');
  ...
  sections.append(
    crudSection({
      ...
      onAdd: (values) => api.addOwnedItem(user.networkUid, values as never),
      onUpdate: (key, values) => api.updateOwnedItem(user.networkUid, Number(key), values as never),
    ...
    crudSection({
      ...
      onAdd: (values) => api.addInventory(user.networkUid, values as never),
      onUpdate: (key, values) => api.updateInventory(user.networkUid, Number(key), values as never),
    ...
    crudSection({
      ...
      valuesOf: (item) => ({ ...item }),
      keyOf: (item) => String(item.globalItemId),
      onAdd: (values) => api.addIngredient(user.networkUid, values as never),
      onUpdate: (key, values) => api.updateIngredient(user.networkUid, Number(key), values as never),
    ...
    crudSection({
      ...
      onAdd: (values) => api.addGardenPlot(user.networkUid, values as never),
      onUpdate: (key, values) => api.updateGardenPlot(user.networkUid, Number(key), values as never),
    ...
    crudSection({
      ...
      renderRow: (item) => {
        let tileCount = 0;
        try {
          tileCount = (JSON.parse(item.tilesJson) as number[]).length;
        } catch {
          tileCount = 0;
        }
    ...
    crudSection({
      ...
      onAdd: (values) => api.addEmployee(user.networkUid, values as never),
      onUpdate: (key, values) => api.updateEmployee(user.networkUid, key, values as never),
    ...
    crudSection({
      ...
      fields: MAIL_FIELDS,
      valuesOf: (item) => ({ ...item, recipientNetworkUid: playerLabel(item.recipientNetworkUid), globalItemIds: (JSON.parse(item.globalItemIdsJson || '[]') as number[]).join(', ') }),
    ...
    crudSection({
      ...
      fields: MAIL_FIELDS,
      valuesOf: (item) => ({ ...item, senderNetworkUid: playerLabel(item.senderNetworkUid), globalItemIds: (JSON.parse(item.globalItemIdsJson || '[]') as number[]).join(', ') }),
      keyOf: (item) => String(item.id),
      onAdd: (values) => api.addMail(user.networkUid, parseMailValues(values)),
      onUpdate: (key, values) => api.updateMail(user.networkUid, Number(key), parseMailValues(values)),
      onDelete: (key) => api.deleteMail(user.networkUid, Number(key)),
    ...
    crudSection({
      ...
      valuesOf: (item) => ({ ...item }),
      keyOf: (item) => String(item.id),
      onAdd: (values) => api.addGameEvent(user.networkUid, values as never),
      onDelete: (key) => api.deleteGameEvent(user.networkUid, Number(key)),
      noEdit: true,
    }),
    imagesSection(user),
    otherRecordsSection(user),
  );

  container.textContent = '';
  container.append(
    also(h('button', { class: 'rc-btn small', type: 'button' }, '← Back to players'), (btn) => btn.addEventListener('click', () => (window.location.hash = '#/users'))),
    header,
...
function staleIdBanner(user: AdminUser): HTMLElement | null {
  const known = new Set(catalog.map((entry) => entry.id));
  const found: Array<{ section: string; id: number }> = [];
  const note = (section: string, rows: ReadonlyArray<{ globalItemId: number }>) => {
    for (const row of rows) {
      if (!known.has(row.globalItemId)) found.push({ section, id: row.globalItemId });
    }
  };
  ...
  for (const plot of user.gardenPlots) {
    if (!known.has(plot.ingredientId)) found.push({ section: 'Garden', id: plot.ingredientId });
  }
  for (const mail of [...user.mailsSent, ...user.mailsReceived]) {
    try {
      for (const id of JSON.parse(mail.globalItemIdsJson || '[]') as number[]) {
        if (!known.has(id)) found.push({ section: 'Mail', id });
      }
    } catch {
  ...
  return h('div', { class: 'rc-panel rc-warn-banner' },
    h('b', {}, `Stale item ids (${unique.length})`),
    h('p', {}, 'This player has rows whose item ids are not in the current item databases — leftover ids from early saves. They show as "Unknown item" and the game cannot use them. Edit each row (pick a real item from the combobox) or delete it.'),
    h('div', { class: 'rc-chips' }, ...unique.slice(0, 24).map((entry) => h('span', { class: 'rc-badge warn' }, `${entry.section} · ${entry.id}`))),
  );
}

function openProfileEditor(user: AdminUser, refresh: () => Promise<void>): void {
  const form = renderForm(PROFILE_FIELDS, { ...user }, async (values) => {
    await api.updateUser(user.networkUid, values as never);
    toast('Profile saved');
    refresh();
  }, 'Save profile');
...
function mailItems(globalItemIdsJson: string): Node {
  let ids: number[] = [];
  try {
    ids = JSON.parse(globalItemIdsJson || '[]') as number[];
  } catch {
    ids = [];
  }
...
function parseMailValues(values: Record<string, unknown>): Record<string, unknown> {
...
interface CrudOptions<T> {
  ...
  onUpdate?: (key: string, values: Record<string, unknown>) => Promise<unknown>;
  onDelete: (key: string) => Promise<unknown>;
  noEdit?: boolean;
}

function crudSection<T>(options: CrudOptions<T>): HTMLElement {
  ...
  const panel = h('section', { class: 'rc-panel' },
    h('div', { class: 'rc-panel-head' },
      h('h2', {}, title, h('span', { class: 'rc-dim' }, ` ${fmt(data.length)}`)),
      h('button', { class: 'rc-btn small primary', type: 'button' }, '+ Add').also((btn) => btn.addEventListener('click', () => openEditor(null))),
    ),
  );
  panel.append(table(headers, data.map((item) => [
    ...
    h('span', { class: 'rc-row-actions' },
      ...
      also(h('button', { class: 'rc-btn small danger', type: 'button' }, 'Delete'), (btn) => {
        btn.addEventListener('click', async () => {
          if (!(await confirmDialog('Delete?', `Remove this ${title.toLowerCase()} entry?`, true))) return;
...
function imagesSection(user: AdminUser): HTMLElement {
  const panel = h('section', { class: 'rc-panel' },
    h('div', { class: 'rc-panel-head' }, h('h2', {}, 'Stored images', h('span', { class: 'rc-dim' }, ` ${fmt(user.storedImages.length)}`))),
  );
  ...
  for (const image of user.storedImages) {
    ...
    grid.append(
      h('figure', {},
        h('img', { src: url, alt: `image type ${image.imageType}` }),
        h('figcaption', {}, `type ${image.imageType} · ${image.width}×${image.height} · ${isoTime(image.createdAt)}`),
      ),
    );
  }
  panel.append(grid);
...
function otherRecordsSection(user: AdminUser): HTMLElement {
  const groups: Array<[string, ReadonlyArray<Record<string, unknown>>]> = [
    ...
    ['Cash transactions', user.cashTransactions as unknown as ReadonlyArray<Record<string, unknown>>],
  ];
  const panel = h('section', { class: 'rc-panel' },
    h('div', { class: 'rc-panel-head' }, h('h2', {}, 'Other records')),
  );
  for (const [label, rows] of groups) {
    const details = h('details', { class: 'rc-json' },
      h('summary', {}, `${label} — ${fmt(rows.length)}`),
      h('pre', {}, rows.length ? JSON.stringify(rows.slice(0, 50), null, 2) : 'none'),
    );
...
// ---------- field specs ----------
const PROFILE_FIELDS: ReadonlyArray<FieldSpec> = [
...
const OWNED_FIELDS: ReadonlyArray<FieldSpec> = [
...
const FLOOR_FIELDS: ReadonlyArray<FieldSpec> = [
...
const EMPLOYEE_FIELDS: ReadonlyArray<FieldSpec> = [
...
const MAIL_FIELDS: ReadonlyArray<FieldSpec> = [
...
const EVENT_FIELDS: ReadonlyArray<FieldSpec> = [
