...
export async function render(container: HTMLElement): Promise<void> {
  ...
  const card = (label: string, value: string, sub = '') => h('div', { class: 'rc-card' }, h('div', { class: 'rc-card-label' }, label), h('div', { class: 'rc-card-value' }, value), sub ? h('div', { class: 'rc-card-sub' }, sub) : null);

  const cards = h('div', { class: 'rc-cards' });
  cards.append(
    card('Static files', fmt(data.staticFiles), data.servesRebuiltGameSwf ? 'rebuilt game.swf active' : 'original game.swf'),
  ...
  const actions = h('div', { class: 'rc-toolbar' },
    ...
    also(h('button', { class: 'rc-btn', type: 'button' }, 'Clear request buffer'), (btn) => {
      btn.addEventListener('click', async () => {
        if (!(await confirmDialog('Clear the request buffer?', 'This only clears the in-memory request log shown on the Traffic page.', false))) return;
  ...
  const online = h('section', { class: 'rc-panel' },
    h('h2', {}, 'Online players'),
    el('table', { class: 'rc-table' },
      h('thead', {}, h('tr', {}, h('th', {}, 'Username'), h('th', {}, 'Network UID'), h('th', {}, 'Playfish UID'), h('th', {}, 'Last seen'), h('th', {}, 'Events'))),
      h('tbody', {}, ...(onlineRows.length ? onlineRows.map((row) => h('tr', {}, ...row.map((cell) => h('td', {}, cell)))) : [h('tr', {}, h('td', { colspan: '5', class: 'rc-empty' }, 'No players online right now.'))])),
    ),
  );
