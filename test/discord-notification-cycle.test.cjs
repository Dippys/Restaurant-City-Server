...
process.env.RC_DISCORD_NOTIFICATION_BATCH_SIZE = '1';

const originalFetch = global.fetch;
const sentMessages = [];
let failUserDm = false;
global.fetch = async (input, init) => {
  if (failUserDm) throw new Error('DMs disabled');
...
test.afterEach(() => { failUserDm = false; });
...
test('notification cycle sends all new mail and edge-triggered employee/garden alerts once', async () => {
  ...
  await prisma.account.create({ data: {
    id: 'notify-account-2', username: 'notifychef2', usernameKey: 'notifychef2', firstName: 'Notify', lastName: 'Two',
    pinHash: 'test', pinSalt: 'test', networkUid: '799000004', playfishUid: 799000004,
    discordIdentity: { create: { discordUserId: '123456789012345679', username: 'notifydiscord2', globalName: 'Notify Two' } },
  } });
  await prisma.userProfile.create({ data: {
    id: 'facebook:799000004', networkUid: '799000004', playfishUid: 799000004, firstName: 'Notify', fullName: 'Notify Two', restaurantName: 'Second Kitchen',
  } });
  await prisma.userProfile.create({ data: {
...
test('failed user DMs are silent and advance durable notification state', async () => {
  const now = new Date('2026-09-03T12:00:00.000Z');
  const mail = await prisma.mail.create({ data: {
    senderProfileId: 'facebook:799000003', recipientProfileId: 'facebook:799000001', senderNetworkUid: '799000003', senderPlayfishUid: 799000003,
    recipientNetworkUid: '799000001', recipientPlayfishUid: 799000001, message: 'This DM will be unavailable.', sendDate: Math.floor(now.getTime() / 1000), type: 1,
  } });
  failUserDm = true;
  await assert.doesNotReject(runDiscordNotificationCycle(now));
  failUserDm = false;
  const state = await prisma.discordNotificationState.findUniqueOrThrow({ where: { accountId: 'notify-account' } });
  assert.equal(state.lastMailId, mail.id);
  const before = sentMessages.length;
  await runDiscordNotificationCycle(now);
  assert.equal(sentMessages.length, before, 'failed direct messages must not retry forever');
});
